...
class ZipFileContents:
    def __init__(self, path, filename):
        ...
        try:
            self.zip_ref = zipfile.ZipFile(os.path.join(self.path, filename), 'r')
        except FileNotFoundError as e:
            print(f"Error opening zip file {os.path.join(self.path, filename)}: {e}")
            raise
    ...
    def __get_all_series_from_zip(self):
        ...
        for dcm in self.dicom_files:
            ...
            # create a new series object if it does not exist and add it to the series dictionary
            ...
            match tmptype:
                case "CT":
                    ...
                    self.series_dict[s_name].set_rescale_slope(f.RescaleSlope)
                    self.series_dict[s_name].set_rescale_intercept(f.RescaleIntercept)
                case "RT":
